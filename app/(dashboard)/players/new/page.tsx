...
export default async function NewPlayerPage() {
  ...
  if (!allowed) redirect("/players");

  const activeTeams = await getTeams(supabase, activeOrg.organizationId);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        <CreatePlayerForm organizationId={activeOrg.organizationId} teams={activeTeams} />
      </div>
    </div>
  );
}
